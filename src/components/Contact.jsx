    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await fetch(
                "https://myportfolio-backend-yv2n.onrender.com/api/contact/createQuery",
                {
                    method: "POST",
                    headers: {
        ...
        <section id="contact" className="bg-tertiary px-5 py-10 text-black">
            ...
            <div className="flex flex-col md:flex-row justify-around px-10 py-5 text-2xl">
                <div className="md:w-1/3 md:pr-8 py-11">
                    ...
                    <p id="details" className="py-2 font-bold font-text"><span className="font-bold">Name :</span> {config.name}</p>
                    <p className="py-2 font-bold font-text"><span className="font-bold">Email :</span> {config.email}</p>
                    <p className="py-2 font-bold font-text"><span className="font-bold">Phone :</span> {config.phone}</p>
                    <p className="py-2 font-bold font-text"><span className="font-bold">Address :</span> {config.address}</p>
                </div>
                ...
                <div id="form" className="md:w-1/3 md:pr-8 p-5 px-5  bg-secondary rounded-lg shadow-lg">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <button
                            type="submit"
                            className="w-full py-2 px-4 bg-tertiary text-black font-semibold rounded-md hover:bg-primary focus:outline-none focus:ring-2 focus:ring-black focus:ring-offset-2"
                        >
